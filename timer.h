...
#ifndef PERF_TIMER_ON
#define PERF_TIMER_ON
#endif
...
class PerfCounter {
 public:
  ...
  static std::string Report() {
    ...
    spin_lock.Unlock();
    DrawTable::DrawTB(ss, {25, 20, 20, 20}, {"Name", "Mean", "P99", "now"}, vec);
    return ss.str();
  }
};
...
class Timer {
 ...
 public:
  ...
  static std::string Report() {
    ...
    for (auto key = orderedKeyList.begin(); key != orderedKeyList.end(); ++key) {
      vec.push_back({*key, beautifyNs(map[*key]->mean()), beautifyNs(map[*key]->p(50)),
                     beautifyNs(map[*key]->p(99))});
    }
    spin_lock.Unlock();
    DrawTable::DrawTB(ss, {25, 20, 20, 20}, {"Name", "Mean", "P50", "P99"}, vec);
    return ss.str();
  }
